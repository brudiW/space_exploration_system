...
export class PFD2CDRController {
    ...
    update() {
        ...
        this.addTouchZone("menu", 310, 575, 30, 25, (touch, orbiter) => {
            (async () => {
                await fetch("/api/ov/cdr_pfd2", {
                    method: "POST",
    ...
    menuInterface() {
        ...
        if (this.orbiter != null) {
            ...
            this.screen.addText("ABORT SYSTEM", 15, 70);
            this.addTouchZone("abortsystem", 15, 65, 300, 20, (touch, orbiter) => {
                (async () => {
                    await fetch("/api/ov/cdr_pfd2", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ tab: "abortSystem" })
                    })
                })()
            })

            this.screen.addText("APU/HYD", 15, 90);
            this.addTouchZone("apu", 15, 85, 300, 20, (touch, orbiter) => {
                (async () => {
                    await fetch("/api/ov/cdr_pfd2", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ tab: "apu" })
                    })
                })()
            })

            this.screen.addText("AVIONICS", 15, 110);
            this.addTouchZone("avionics", 15, 105, 300, 20, (touch, orbiter) => {
                (async () => {
                    await fetch("/api/ov/cdr_pfd2", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
            ...
            this.screen.addText("GPC", 15, 130);
            this.addTouchZone("gpc", 15, 125, 300, 20, (touch, orbiter) => {
                (async () => {
                    await fetch("/api/ov/cdr_pfd2", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
            ...
            this.screen.addText("RCS/OMS", 15, 230);
            this.addTouchZone("rcs_oms", 15, 225, 300, 20, (touch, orbiter) => {
                (async () => {
                    await fetch("/api/ov/cdr_pfd2", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
            ...
            this.screen.addText("SSME, ET, SRB", 15, 370);
            this.addTouchZone("ssme", 15, 365, 300, 20, (touch, orbiter) => {
                (async () => {
                    await fetch("/api/ov/cdr_pfd2", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
    ...
    abortSystemInterface() { // CLAS
        ...
        if (this.orbiter != null) {
            this.screen.addFilledRect("darkslategray", 0, 0, 400, 25);
            // Texts A
            this.screen.addText("CLAS", 10, 7);
            this.screen.addText("ARMED", 15, 30);
            this.screen.addText("TRIGGERED", 15, 50);
            this.screen.addText("FAILED", 15, 70);

            this.screen.addText(OV.computers.clasComputer.armState, 110, 30);
            this.screen.addText(OV.computers.clasComputer.triggerState, 110, 50);
            this.screen.addText(OV.computers.clasComputer.failState, 110, 70);


            // Texts B
            this.screen.addText("TRIM [X, Z]", 170, 50);
            this.screen.addText(OV.computers.clasComputer.trim[0], 260, 50);
            this.screen.addText(OV.computers.clasComputer.trim[1], 310, 50);
            this.screen.addText("△", 257, 30);
            this.screen.addText("△", 307, 30);
            this.screen.addText("▽", 257, 70);
            this.screen.addText("▽", 307, 70);
            this.addTouchZone("clasTrimX+", 250, 26, 20, 20, (touch, orbiter) => { OV.computers.clasComputer.trim[0]++; })
            this.addTouchZone("clasTrimX-", 250, 64, 20, 20, (touch, orbiter) => { OV.computers.clasComputer.trim[0]--; })
            this.addTouchZone("clasTrimZ+", 300, 26, 20, 20, (touch, orbiter) => { OV.computers.clasComputer.trim[1]++; })
            this.addTouchZone("clasTrimZ-", 300, 64, 20, 20, (touch, orbiter) => { OV.computers.clasComputer.trim[1]--; })

            // Texts C
            this.screen.addLine("lime", 0, 90, 400, 90)
    ...
    ssmeInterface() {
        ...
        if (this.orbiter != null) {
            ...
            if (!OV.et.jettisoned && !OV.et.TERMINATED) {
            ...
            if (!OV.SRBs.l.seperated && !OV.SRBs.l.TERMINATED) {
            ...
            if (!OV.SRBs.r.seperated && !OV.SRBs.r.TERMINATED) {
